...
const Profile = () => {
  ...
  const loadUser = async () => {
    try {
      const res = await fetch(`http://localhost:8080/api/users/${userLocal.id}`);
  ...
  const loadConges = async () => {
    try {
      const res = await fetch(`http://localhost:8080/api/conges/agent/${userLocal.id}`);
      const data = await res.json();
  ...
  const handleChangePassword = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:8080/api/users/${userLocal.id}/password`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
...
if (res.ok) {
  ...
  const handleAddConge = async () => {
    ...
    const res = await fetch("http://localhost:8080/api/conges", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  const handleDelete = async (id) => {
    const res = await fetch(`http://localhost:8080/api/conges/${id}`, {
      method: "DELETE"
    });
